...
class NewsCard {
    public static function render($id, $header, $imageURL, $content) {
        ...
        <div class="max-w-sm bg-gray-800 rounded-lg shadow-lg overflow-hidden m-auto">
          ...
          <a href="/dwp/news/<?php echo htmlspecialchars($id); ?>">
            <img class="h-[260px] object-cover object-center" src="src/assets/<?php echo htmlspecialchars($imageURL); ?>" alt="Movie Poster">
          </a>
          <!-- Content -->
          <div class="p-4">
            <a href="/dwp/news/<?php echo htmlspecialchars($id)?>" class="text-xl font-semibold text-white mb-2"><?php echo htmlspecialchars($header); ?></a>

            <p class="text-gray-400 text-sm leading-6">
              <?php echo htmlspecialchars($shortenedContent); ?>
              <a href="/dwp/news/<?php echo htmlspecialchars($id); ?>" class="text-blue-500 hover:underline">Read more</a>
            </p>
          </div>
        </div>
        <?php
