...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        get($baseRoute . '/genres', function() use ($movieController) {
                $movieController = new MovieController();
                ...
                if (!empty($genres)) {
                    ...
                    echo json_encode(['error' => 'No genres found.']);
                }
            });
        {
            try {
                const response = await fetch($baseRoute . '/genres');
            ...
            data.forEach(genre => {
                ...
                wrapper.className = 'genre-checkbox-item flex items-center gap-2';
                wrapper.appendChild(checkbox);
                wrapper.appendChild(label);

                addGenresContainer.appendChild(wrapper);
            });
        }
    .catch(error => {
